...
async def audit_middleware(request: Request, call_next):
    ...
    try:
        ...
        logger.debug(f"[audit] Failed to parse auth header: {e}")
...
def get_status(chapter_id: int, db: Session = Depends(get_db), user: str = Depends(require_admin)):
    chapter = (
        db.query(models.Chapter)
...
def download_zip(chapter_id: int, db: Session = Depends(get_db), user: str = Depends(require_admin)):
    chapter = (
        db.query(models.Chapter)
        .join(models.Series, models.Series.id == models.Chapter.series_id)
        .filter(models.Chapter.id == chapter_id)
...
def get_json(chapter_id: int, db: Session = Depends(get_db), user: str = Depends(require_admin)):
